...
// ========================================
// AI Movie Recommender functions (Gemini) (Unchanged)
// ========================================
...
if (aiSendBtn) {
    aiSendBtn.addEventListener('click', async () => {
        ...
        if (!GEMINI_API_KEY || GEMINI_API_KEY.includes("YOUR_")) {
            addMessageToChat('assistant', "⚠️ **ERROR:** Please add your **Gemini API Key** to `script.js` to enable the AI chat.");
            return;
        }
        ...
        if (!userMessage) {
            alert('Please enter what you\'re in the mood for!');
            return;
        }
        ...
        aiSendBtn.innerHTML = '<i class="fas fa-spinner fa-spin"></i> Thinking...';
        
        const prompt = `You are a movie recommendation expert named StreamFinder AI. Based on the user's mood/preferences, recommend 3-5 distinct movies or TV shows that would be perfect for them. 
        
        IMPORTANT: You MUST respond with ONLY a JSON array, no other text before or after. Each item must be a movie/show title and a brief reason.
        
        User's request: "${userMessage}"
        
        Respond with ONLY valid JSON array like this (no markdown, no backticks, no preamble):
        [{"title":"Movie Name","reason":"Brief reason why it fits the mood."},{"title":"Another Movie","reason":"Another reason."}]`;
        ...
        try {
            ...
            if (data.error || !data.candidates) {
                 addMessageToChat('assistant', `⚠️ API Error: Could not get a response from Gemini. Check API key or console for details.`);
                 console.error("Gemini Error Detail:", data.error || data);
                 return;
            }
            ...
            try {
                ...
                addMessageToChat('assistant', 'I had trouble formatting the recommendations. The AI response was garbled.');
                return;
            }
            
            const movieTitles = recommendations.map(r => r.title).join(', ');
            addMessageToChat('assistant', `Great! Based on your mood, I recommend: ${movieTitles}. Let me find these for you...`);
            
            await searchAndDisplayAIMovies(recommendations);
            
        } catch (error) {
            console.error('AI API Fetch Error:', error);
            addMessageToChat('assistant', 'Sorry, I encountered a network error. Please try again!');
        }
