...
document.querySelectorAll('.nav-btn-home').forEach(btn => {
    btn.addEventListener('click', (e) => {
        ...
        if (navTarget === 'ai') switchView('ai');
        else if (navTarget === 'favorites') switchView('favs');
        else if (navTarget === 'trending') switchView('trending');
    });
});

// Update watchlist count on home page
function updateWatchlistCountDisplay() {
    const countDisplay = document.getElementById('watchlist-count-display');
    if (countDisplay) {
        countDisplay.textContent = `${favorites.length} movie${favorites.length !== 1 ? 's' : ''} saved for later. Browse and manage your collection.`;
    }
}
...
// ========================================
// TMDB-Based Movie Recommendation Chatbot
// ========================================
...
// Mood/Genre mappings for intelligent searching
const CHATBOT_MOOD_MAP = {
    cozy: { genres: [35, 10749], keywords: ['cozy', 'warm', 'comfortable', 'relaxing', 'feel-good'] },
    happy: { genres: [35], keywords: ['happy', 'funny', 'laugh', 'comedy', 'cheerful'] },
    excited: { genres: [28, 53, 878], keywords: ['excited', 'action', 'adventure', 'thrilling', 'fast-paced'] },
    sad: { genres: [18], keywords: ['sad', 'emotional', 'drama', 'cry', 'touching'] },
    scared: { genres: [27, 53], keywords: ['scary', 'horror', 'terrifying', 'thriller', 'suspense'] },
    tired: { genres: [35, 10751], keywords: ['tired', 'light', 'easy', 'relax', 'chill'] },
    curious: { genres: [99, 9648], keywords: ['curious', 'documentary', 'mystery', 'interesting', 'learning'] },
    romantic: { genres: [10749], keywords: ['romantic', 'love', 'romance', 'couple', 'relationship'] },
    adventurous: { genres: [12, 878], keywords: ['adventure', 'explore', 'journey', 'fantasy', 'sci-fi'] }
};

// Get the detected mood from user input
function detectMoodFromInput(userInput) {
    const lowerInput = userInput.toLowerCase();
    
    for (const [mood, data] of Object.entries(CHATBOT_MOOD_MAP)) {
        if (data.keywords.some(keyword => lowerInput.includes(keyword))) {
            return { mood, genres: data.genres };
        }
    }
    
    // Default mood if no match
    return { mood: 'cozy', genres: [35, 10749] };
}

// Generate a friendly chatbot response
function generateChatbotResponse(detectedMood) {
    const responses = {
        cozy: "Aww, sounds like you need something warm and comforting! Let me find the perfect cozy movies for you... ☕",
        happy: "I love your energy! Let me find something hilarious to keep that smile going! 😄",
        excited: "YES! Let's get that adrenaline pumping with some action-packed movies! 🚀",
        sad: "I'm here for you. Let me find some emotionally touching films that might help... 💙",
        scared: "Ready to get spooked? I've got some great thrillers and horror picks for you! 👻",
        tired: "Time to relax! Let me find something light and easy to watch... 😴",
        curious: "I love your curiosity! Let me find some fascinating documentaries and mysteries... 🔍",
        romantic: "Aww, sounds romantic! Let me find some beautiful love stories... 💕",
        adventurous: "Adventure awaits! Let me find some epic journeys for you... 🌍"
    };
    
    return responses[detectedMood] || responses.cozy;
}
...
if (aiSendBtn) {
    aiSendBtn.addEventListener('click', async () => {
        ...
        aiSendBtn.innerHTML = '<i class="fas fa-spinner fa-spin"></i> Finding movies...';
        
        // Detect mood and get response
        const { mood, genres } = detectMoodFromInput(userMessage);
        const assistantResponse = generateChatbotResponse(mood);
        
        addMessageToChat('assistant', assistantResponse);
        
        // Search for movies based on detected mood
        await searchMoviesByMood(genres, mood);
...
document.querySelectorAll('.mood-btn').forEach(btn => {
    btn.addEventListener('click', async () => {
        ...
        const moodData = CHATBOT_MOOD_MAP[mood];
        
        // Create a friendly message for the user
        const moodText = btn.textContent.split(' ').slice(1).join(' ');
        addMessageToChat('user', moodText);
        ...
        // Search for movies
        await searchMoviesByMood(moodData.genres, mood);
    });
});

// Search TMDB for movies based on genres
async function searchMoviesByMood(genreIds, mood) {
    aiResults.innerHTML = '<div class="ai-loading"><i class="fas fa-spinner fa-spin"></i> Searching TMDB...</div>';
    ...
    try {
        ...
        if (!data.results || data.results.length === 0) {
            aiResults.innerHTML = '<p style="color: var(--text-muted); text-align: center;">No movies found. Try a different mood!</p>';
            return;
        }
        ...
        // Display results
        displayAIMovieResults(enrichedMovies);
...
// Display the movie results in a grid
function displayAIMovieResults(movies) {
    aiResults.innerHTML = '';
...
async function searchAndDisplayAIMovies(recommendations) {
    aiResults.innerHTML = '<div class="ai-loading"><i class="fas fa-spinner fa-spin"></i> Searching for your perfect matches...</div>';
    ...
    for (const rec of recommendations) {
        try {
            ...
            searchUrl.searchParams.append('query', rec.title);
            ...
            if (data.results && data.results.length > 0) {
                ...
                movie.ai_reason = rec.reason;
                const enriched = await enrichMovieData(movie);
                foundMovies.push(enriched);
            }
        } catch (error) {
    ...
    if (foundMovies.length === 0) {
        aiResults.innerHTML = '<div class="empty-state">Couldn\'t find those movies. Try describing your mood differently!</div>';
        return;
    }
    ...
    foundMovies.forEach(movie => {
        const card = createAIMovieCard(movie);
        resultsGrid.appendChild(card);
    });

    aiResults.appendChild(resultsGrid);
}

function createAIMovieCard(movie) {
    ...
    if (movie.ai_reason) {
        ...
        if (details) {
            ...
            reasonDiv.innerHTML = `<strong style="color: var(--ai-green);">🤖 AI Pick:</strong> ${movie.ai_reason}`;
            details.insertBefore(reasonDiv, details.firstChild);
        }
    }
...
function createMovieCard(movie) {
    ...
    // 4. IMDb link (Prevents card expansion/collapse when clicking through)
    ...
    // 5. FIX: Actor Click Handler (Opens IMDb page)
